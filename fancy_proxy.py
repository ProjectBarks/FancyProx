...
class MirroredContent(object):
    ...
    def get_by_key_name(key_name):
        result = memcache.get_multi(['%s.%s' % (key_name, i) for i in range(32)])
    ...
    def fetch_and_store(key_name, base_url, translated_address, mirrored_url):
        ...
        try:
            ...
            logging.exception("Could not fetch URL URL: %s", mirrored_url)
            return None
        ...
        new_content = MirroredContent(
            ...
            data=content)
        serialized = pickle.dumps(new_content, 2)
        values = {}
        for i in range(0, len(serialized), MAX_CONTENT_SIZE):
            values['%s.%s' % (key_name, i // MAX_CONTENT_SIZE)] = serialized[i: i + MAX_CONTENT_SIZE]
...
class MirrorHandler(BaseHandler):
    def get(self, base_url):
        ...
        logging.info("Handling request for %s %s %s %s", key_name, base_url, mirrored_url, translated_address)
